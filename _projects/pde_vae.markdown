...
date: 2020-09-11
img: /assets/img/pde_vae_key_image.png
...
class DynamicConv1d(nn.Module):
    ...
    def forward(self, input, weight, bias):
        ...
        image_patches = y.unfold(-2, self.kernel_size[0], self.stride[0]).transpose(-1, -2).contiguous()
        y = torch.matmul(   image_patches.view(-1, output_size, self.in_channels * self.kernel_size[0]),
                            weight.view(-1, self.in_channels * self.kernel_size[0], self.out_channels)
                        )
        if bias is not None:
...
class ConvPropagator(nn.Module):
    def __init__(self, hidden_channels, linear_kernel_size, nonlin_kernel_size, data_channels, stride=1,
        ...
        if prop_layers > 0:
            self.conv_prop = nn.ModuleList([DynamicConv1d(hidden_channels, hidden_channels, nonlin_kernel_size, stride,
                                            nonlin_padding, dilation, groups, boundary_cond)
